...
int rawRead(unqlite_file *id, void *pBuf, unqlite_int64 amt, unqlite_int64 offset)
{
	...
	VFS_DEBUG_START();
	VFS_DEBUG_MSG("READ file=%p, buffer=%p, amount=%d, offset=%lld\n", pFile->h, pBuf, amt, offset);
    ...
    // a seek beyond the end of the file is performed. This causes SQLite to interpret
    // the database as corrupted.
	if (offset < fileSize)
	{
	...
	VFS_DEBUG_FINALIZE("READ file=%p, rc=UNQLITE_OK -> ", pFile->h);
...
int rawWrite(unqlite_file *id, const void *pBuf, unqlite_int64 amt, unqlite_int64 offset)
{
	...
	VFS_DEBUG_START();
	VFS_DEBUG_MSG("WRITE file=%p, buffer=%p, amount=%d, offset=%lld\n", pFile->h, pBuf, amt, offset);
	...
	if (rc != 0)
	{
		VFS_DEBUG_MSG("WRITE file=%p, rc=UNQLITE_FULL\n", pFile->h);
		return UNQLITE_FULL;
	}
	VFS_DEBUG_FINALIZE("SEEK -> ");
	...
	if (rc != 0)
	{
		VFS_DEBUG_MSG("WRITE file=%p, rc=UNQLITE_IOERR\n", pFile->h);
		return UNQLITE_IOERR;
	}
	...
	VFS_DEBUG_MSG("WRITE file=%p, rc=UNQLITE_OK\n", pFile->h);
	VFS_DEBUG_FINALIZE("WR -> ");
...
int rawFileSize(unqlite_file *id, unqlite_int64 *pSize)
{
	...
	*pSize = info.st_size;
	...
	VFS_DEBUG_MSG("SIZE file=%p, pSize=%p, *pSize=%lld\n", pFile->h, pSize, *pSize);
	return UNQLITE_OK;
}
...
*            SQLite code assumes this function cannot fail. It also assumes that if two files are
*            created in the same file-system directory (i.e. a database and its journal file)
*            that the sector size will be the same for both.
