...
CPU_BOOLEAN  Shell_Init (void)
{
    ...
    SHELL_MODULE_CMD  *pmodule_cmd;
                                                                ...
                                                                /* ---------------- INIT SHELL CMD TBL ---------------- */
    Shell_ModuleCmdUsedPoolPtr = (SHELL_MODULE_CMD *)0;         /* Init-clr module cmd pools    (see Note #3).          */
    Shell_ModuleCmdFreePoolPtr = (SHELL_MODULE_CMD *)0;
    ...
    for (i = 0; i < SHELL_CFG_CMD_TBL_SIZE; i++) {              /* Free each module cmd to pool (see Note #3).          */
        ...
        pmodule_cmd->PrevModuleCmdPtr = (void *)0;
        pmodule_cmd->NextModuleCmdPtr = (void *)Shell_ModuleCmdFreePoolPtr;

        if (Shell_ModuleCmdFreePoolPtr != (SHELL_MODULE_CMD *)0) {
            Shell_ModuleCmdFreePoolPtr->PrevModuleCmdPtr = (void *)pmodule_cmd;
        }
                       ...
                       SHELL_ERR  *perr)
{
    ...
    ptbl_name = (CPU_CHAR *)0;

    if (cmd_tbl_name != (CPU_CHAR *)0) {                        /* If cmd_tbl_name not null ...                         */
    ...
    if (ptbl_name == (CPU_CHAR *)0) {                           /* If cmd tbl name not gotten from param ...            */
        pcmd = &cmd_tbl[0];
        Shell_ModuleCmdNameGet(pcmd->Name,                      /* ... get name from first command.                     */
            tbl_name,
    ...
    while (pmodule_cmd != (SHELL_MODULE_CMD *)0) {
        ...
        pmodule_cmd = pmodule_cmd->NextModuleCmdPtr;
                                                                ...
                                                                /* ------------------ GET MODULE CMD ------------------ */
    if (Shell_ModuleCmdFreePoolPtr != (SHELL_MODULE_CMD *)0) {  /* If module cmd pool NOT empty ...                     */
                                                                /* ... get one from pool.                               */
        pmodule_cmd                 = (SHELL_MODULE_CMD *)Shell_ModuleCmdFreePoolPtr;
        Shell_ModuleCmdFreePoolPtr  = (SHELL_MODULE_CMD *)pmodule_cmd->NextModuleCmdPtr;
    ...
    pmodule_cmd->PrevModuleCmdPtr = (void *)0;
    pmodule_cmd->NextModuleCmdPtr = (void *)Shell_ModuleCmdUsedPoolPtr;

    if (Shell_ModuleCmdUsedPoolPtr != (SHELL_MODULE_CMD *)0) {
        Shell_ModuleCmdUsedPoolPtr->PrevModuleCmdPtr = (void *)pmodule_cmd;
    }
                       ...
                       SHELL_ERR  *perr)
{
    ...
    CPU_INT16S         name_compare;
                                                                ...
                                                                /* ------------------- VALIDATE PTR ------------------- */
    if (cmd_tbl_name == (CPU_CHAR *)0) {
       *perr = SHELL_ERR_NULL_PTR;
        return;
    }

                                                                /* ----------------- SEARCH MODULE CMD ---------------- */  
    pmodule_cmd_list = (SHELL_MODULE_CMD *)Shell_ModuleCmdUsedPoolPtr;
    pmodule_cmd      = (SHELL_MODULE_CMD *)0;

    while (pmodule_cmd_list != (SHELL_MODULE_CMD *)0) {
    ...
    if (pmodule_cmd == (SHELL_MODULE_CMD *)0) {
    ...
    if (pmodule_cmd->PrevModuleCmdPtr != (void *)0) {           /* If prev NOT NULL ... set prev's next to next.        */
        ((SHELL_MODULE_CMD *)pmodule_cmd->PrevModuleCmdPtr)->NextModuleCmdPtr = pmodule_cmd->NextModuleCmdPtr;
    } else {                                                    /* Else ...             set used pool ptr to next.      */
        Shell_ModuleCmdUsedPoolPtr = pmodule_cmd->NextModuleCmdPtr;
    }

    if (pmodule_cmd->NextModuleCmdPtr != (void *)0) {           /* If next NOT NULL ... set next's prev to prev.        */
        ((SHELL_MODULE_CMD *)pmodule_cmd->NextModuleCmdPtr)->PrevModuleCmdPtr = pmodule_cmd->PrevModuleCmdPtr;
    }
    ...
    if (pmodule_cmd == (SHELL_MODULE_CMD *)0) {
       *perr = SHELL_ERR_MODULE_CMD_NOT_FOUND;
        return ((SHELL_CMD_FNCT)0);
                                                                ...
                                                                /* -------------------- SEARCH CMD -------------------- */
    pcmd_list = (SHELL_CMD *)pmodule_cmd->CmdTblPtr;
    pcmd      = (SHELL_CMD *)0;
...
static  void  Shell_ModuleCmdClr (SHELL_MODULE_CMD  *pmodule_cmd)
{
    ...
    pmodule_cmd->PrevModuleCmdPtr = (void *)0;
    pmodule_cmd->NextModuleCmdPtr = (void *)0;

    pmodule_cmd->CmdTblPtr        = (SHELL_CMD *)0;
}
